// Get Ids:
const ids = (tile, time, player)=>{
    let itemId= `${tile}${time}-${player}`
    return document.getElementById('`${itemId}`')
}
// Id-points player white:

const blueAWhite = document.getElementById('blueA-white');
const redAWhite= document.getElementById('redA-white');
const brownAWhite= document.getElementById('brownA-white');
const whiteAWhite= document.getElementById('whiteA-white');
const greenAWhite= document.getElementById('greenA-white');
const purpleAWhite= document.getElementById('purpleA-white');

const blueB1White = document.getElementById('blueB1-white');
const redB1White = document.getElementById('redB1-white');
const brownB1White= document.getElementById('brownB1-white');
const whiteB1White= document.getElementById('whiteB1-white');
const greenB1White= document.getElementById('greenB1-white');
const purpleB1White= document.getElementById('purpleB1-white');

const blueB2White = document.getElementById('blueB2-white');
const redB2White = document.getElementById('redB2-white');
const brownB2White= document.getElementById('brownB2-white');
const whiteB2White= document.getElementById('whiteB2-white');
const greenB2White= document.getElementById('greenB2-white');
const purpleB2White= document.getElementById('purpleB2-white');

const blueC1White = document.getElementById('blueC1-white');
const redC1White = document.getElementById('redC1-white');
const brownC1White= document.getElementById('brownC1-white');
const whiteC1White= document.getElementById('whiteC1-white');
const greenC1White= document.getElementById('greenC1-white');
const purpleC1White= document.getElementById('purpleC1-white');

const blueC2White = document.getElementById('blueC2-white');
const redC2White = document.getElementById('redC2-white');
const brownC2White= document.getElementById('brownC2-white');
const whiteC2White= document.getElementById('whiteC2-white');
const greenC2White= document.getElementById('greenC2-white');
const purpleC2White= document.getElementById('purpleC2-white');

const blueC3White = document.getElementById('blueC3-white');
const redC3White = document.getElementById('redC3-white');
const brownC3White= document.getElementById('brownC3-white');
const whiteC3White= document.getElementById('whiteC3-white');
const greenC3White= document.getElementById('greenC3-white');
const purpleC3White= document.getElementById('purpleC3-white');

const totalWhite= document.getElementById('totalWhite');

// Ids for player blue:

const blueABlue = document.getElementById('blueA-blue');
const redABlue= document.getElementById('redA-blue');
const brownABlue= document.getElementById('brownA-blue');
const whiteABlue= document.getElementById('whiteA-blue');
const greenABlue= document.getElementById('greenA-blue');
const purpleABlue= document.getElementById('purpleA-blue');

const blueB1Blue = document.getElementById('blueB1-blue');
const redB1Blue = document.getElementById('redB1-blue');
const brownB1Blue= document.getElementById('brownB1-blue');
const whiteB1Blue= document.getElementById('whiteB1-blue');
const greenB1Blue= document.getElementById('greenB1-blue');
const purpleB1Blue= document.getElementById('purpleB1-blue');

const blueB2Blue = document.getElementById('blueB2-blue');
const redB2Blue = document.getElementById('redB2-blue');
const brownB2Blue= document.getElementById('brownB2-blue');
const whiteB2Blue= document.getElementById('whiteB2-blue');
const greenB2Blue= document.getElementById('greenB2-blue');
const purpleB2Blue= document.getElementById('purpleB2-blue');

const blueC1Blue = document.getElementById('blueC1-blue');
const redC1Blue = document.getElementById('redC1-blue');
const brownC1Blue= document.getElementById('brownC1-blue');
const whiteC1Blue= document.getElementById('whiteC1-blue');
const greenC1Blue= document.getElementById('greenC1-blue');
const purpleC1Blue= document.getElementById('purpleC1-blue');

const blueC2Blue = document.getElementById('blueC2-blue');
const redC2Blue = document.getElementById('redC2-blue');
const brownC2Blue= document.getElementById('brownC2-blue');
const whiteC2Blue= document.getElementById('whiteC2-blue');
const greenC2Blue= document.getElementById('greenC2-blue');
const purpleC2Blue= document.getElementById('purpleC2-blue');

const blueC3Blue = document.getElementById('blueC3-blue');
const redC3Blue = document.getElementById('redC3-blue');
const brownC3Blue= document.getElementById('brownC3-blue');
const whiteC3Blue= document.getElementById('whiteC3-blue');
const greenC3Blue= document.getElementById('greenC3-blue');
const purpleC3Blue= document.getElementById('purpleC3-blue');

const totalBlue= document.getElementById('totalBlue');